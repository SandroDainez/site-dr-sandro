...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const d = await getResearchPublicado("pesquisador", slug);
  if (!d) return { title: "Síntese não encontrada" };
...
export default async function PesquisaPublicaPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const [pesq, header, navItems, typo, navStyle] = await Promise.all([
    getResearchPublicado("pesquisador", slug), getHeader(), getNavItems(), getTypography(), getNavStyle(),
  ]);
  if (!pesq) notFound();

  const secoes = (pesq.conteudo.secoes ?? []).map((s) => s.secao);
  const referencias = pesq.conteudo.referencias ?? [];
    ...
    <div className="min-h-screen bg-[#0f1420] text-white">
      ...
      <header data-typo="header" className="sticky top-0 z-50 border-b border-white/10 bg-[#0f1420]/80 backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-7xl flex-col items-center gap-3 px-6 py-4 lg:flex-row lg:justify-between lg:gap-0">
          ...
          <Link href="/pesquisas" className="flex items-center gap-1 text-sm text-white/50 transition hover:text-white lg:hidden">← Sínteses</Link>
        </div>
      </header>

      <main className="mx-auto w-full max-w-4xl px-6 py-12">
        <Link href="/pesquisas" className="mb-6 inline-flex items-center gap-1.5 text-sm text-white/50 transition hover:text-white">← Todas as sínteses</Link>
        ...
        <div className="mb-6">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-accent">Síntese de evidência · {ESP_LABEL[pesq.specialty] ?? pesq.specialty}</p>
          <h1 className="mt-2 text-3xl font-semibold leading-tight tracking-tight md:text-4xl">{pesq.title}</h1>
          {pesq.publicado_em && <p className="mt-2 text-[12px] text-white/40">Publicado em {dataCurta(pesq.publicado_em)}</p>}
        </div>
            ...
            {(pesq.conteudo.secoes ?? []).map((sec) => (
              <section key={sec.secao} id={slugify(sec.secao)} className="scroll-mt-24">
                ...
                <div className="whitespace-pre-wrap text-[15px] leading-relaxed text-white/80">
                  {textoSecao(sec.secao, pesq.conteudo) || <span className="text-white/30">—</span>}
                </div>
              </section>
            ))}
