...
export const metadata = {
  title: "Atualizações de protocolos",
  description: "Relatórios de atualização de protocolos, com evidências recentes citadas.",
};
...
export default async function AtualizacoesProtocolosPage() {
  const [docs, header, navItems, typo, navStyle] = await Promise.all([
    getAtualizacoesPublicadas(), getHeader(), getNavItems(), getTypography(), getNavStyle(),
  ]);
    ...
    <div className="min-h-screen bg-[#0f1420] text-white">
      ...
      <main className="mx-auto w-full max-w-7xl px-6 py-16">
        <div className="mb-12">
          ...
          <h1 className="mt-3 text-4xl font-medium tracking-tight md:text-5xl">Atualizações de protocolos</h1>
          <p className="mt-3 text-base text-white/50">O que a evidência recente muda (ou reforça) nos protocolos, com fontes citadas.</p>
        </div>

        {docs.length === 0 ? (
          <p className="rounded-2xl border border-white/10 bg-white/[0.03] px-5 py-8 text-center text-sm text-white/40">Nenhuma atualização publicada ainda.</p>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {docs.map((d) => (
              <a key={d.id} href={`/atualizacoes-protocolos/${d.slug}`} className="group flex flex-col gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-5 transition hover:border-accent/40 hover:bg-white/[0.05]">
                ...
                <span className="mt-auto inline-flex items-center gap-1 text-xs font-semibold text-accent">Ver atualização <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-1" /></span>
              </a>
            ))}
          </div>
        )}
