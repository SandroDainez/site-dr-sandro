...
export default function TitulosEditor({ initial }: { initial: SectionTextsData }) {
  ...
  function update(key: string, field: "eyebrow" | "title", value: string) {
  ...
  return (
    <div className="space-y-4">
      {SECTION_TEXTS.map((s) => {
        const eyebrow = data[s.key]?.eyebrow ?? "";
        const title = data[s.key]?.title ?? "";
        const changed = !!(data[s.key]?.eyebrow || data[s.key]?.title);
        return (
          <div key={s.key} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label className="mb-1 block text-xs uppercase tracking-[0.1em] text-white/40">Rótulo pequeno (em cima)</label>
                <input
                  className={inputCls}
                  value={eyebrow}
                  placeholder={s.eyebrow}
                  onChange={(e) => update(s.key, "eyebrow", e.target.value)}
                />
              </div>
              <div>
            ...
            <p className="mt-2 text-[11px] text-white/35">
              Deixe em branco para usar o padrão: <span className="text-white/50">“{s.eyebrow}” / “{s.title}”</span>
            </p>
          </div>
        );
      })}
