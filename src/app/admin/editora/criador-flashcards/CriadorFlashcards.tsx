...
export default function CriadorFlashcards({ docsIniciais, modo }: { docsIniciais: Doc[]; modo: "mock" | "real" }) {
  ...
  async function carregarVersoes(id: string, autoAbrirUltima = false) {
    ...
    if (r.ok) {
      setVersoes(r.data);
      // Ao abrir um baralho, recarrega a versão mais recente no editor (senão vinha vazio).
  ...
  function abrirDoc(d: Doc) {
    setDoc(d); setSecoes([]); setMeta(null); setSalvo(null); setError(null); setRevisao(null); setTextoEditado({});
            ...
            <div ref={resultadoRef} className={card}>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[11px] text-white/40">{validacao.validadas}/{validacao.totalClinicas} respostas que exigem fonte com citação validada · {validacao.semFonte} sem fonte · {validacao.invalidas} inválidas</p>
                </div>
              </div>
