...
import { criarDoc, listarSources, adicionarSource, removerSource, gerar, revisar, salvarVersao, listarVersoes, carregarVersao, excluirVersao, publicarDoc, despublicarDoc, arquivarDoc, excluirDoc, aplicarCorrecoes } from "./actions";
...
type Meta = { provider: string; model: string; tokensIn: number; tokensOut: number; secoes: SecaoGerada[]; confidence: number; method: string };
...
function renderVerso(sec: SecaoGerada): string {
  return sec.afirmacoes
    .map((a) => a.texto)
    .join("\n");
}

export default function CriadorFlashcards({ docsIniciais, modo }: { docsIniciais: Doc[]; modo: "mock" | "real" }) {
  ...
  // Validação MANUAL: o médico marca uma afirmação do verso como "conferi na fonte" (resolve o
  // aviso sem citação verbatim). Fica gravado na versão ao salvar; contado à parte da confiança.
  ...
  async function carregarVersoes(id: string, autoAbrirUltima = false) {
    ...
    if (r.ok) {
      setVersoes(r.data);
      // Ao abrir um baralho, recarrega a versão mais recente no editor (SEM entrar no modo
  ...
  function abrirDoc(d: Doc) {
    setDoc(d); setSecoes([]); setMeta(null); setSalvo(null); setError(null); setRevisao(null); setTextoEditado({}); setEditandoVersao(null);
  ...
  async function gerarTudo() {
    ...
    const r = await gerar({ docId: doc.id, especialidade, quantidade });
    if (!r.ok) { setError(r.error); setGerando(false); return; }
    setSecoes(r.data.secoes);
    setMeta({ provider: r.data.provider, model: r.data.model, tokensIn: r.data.usage.tokensIn, tokensOut: r.data.usage.tokensOut, secoes: r.data.secoes, confidence: r.data.validacao.confidence, method: r.data.validacao.method });
    const te: Record<string, string> = {};
    for (const s of r.data.secoes) te[s.secao] = renderVerso(s);
    setTextoEditado(te);
    setGerando(false);
  }
  ...
  // Aplicar correções da IA: reancora as afirmações do verso reprovadas e recalcula a confiança.
  ...
  function salvar() {
    ...
    const revisaoMeta = revisao
      ...
      : undefined;
    startTransition(async () => {
      const r = await salvarVersao({ docId: doc.id, especialidade, secoes, textoEditado, geracao: meta ?? undefined, revisao: revisaoMeta });
  ...
  return (
    <div className="space-y-6">
      ...
      {editandoVersao != null && (
        <div className="sticky top-2 z-20 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-accent/50 bg-accent/15 px-4 py-2.5 shadow-lg backdrop-blur">
          <p className="inline-flex items-center gap-2 text-sm font-semibold text-accent">
            <PencilLine className="h-4 w-4" /> Editando a Versão {editandoVersao} — altere os cartões abaixo e salve como nova versão.
          </p>
      ...
      {/* 1) BARALHO */}
      <div className={card}>
        <p className="mb-3 text-xs font-semibold uppercase tracking-[0.12em] text-white/40">1 · Baralho</p>
        {doc ? (
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
              <div>
                ...
                <input className={inputCls} value={novoTitulo} onChange={(e) => setNovoTitulo(e.target.value)} placeholder="Título do baralho. Ex.: Sepse — pontos-chave" />
              </div>
            ...
            </div>                <p className="mt-3 text-[11px] text-white/40">Depois de criar (ou abrir um existente acima), a área para colar as <strong className="text-white/60">referências</strong> aparece logo abaixo.</p>
          </div>
        )}
      </div>
          ...
          <div className={card}>
            ...
            <div className="flex flex-wrap items-end gap-3">
              <div>
                ...
                <select className={inputCls + " sm:w-48"} value={especialidade} onChange={(e) => setEspecialidade(e.target.value)}>{ESPECIALIDADES_MODULO.map((e) => <option key={e} value={e}>{e}</option>)}</select>
              </div>
              <div>
            ...
            {meta && (
              <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg border border-white/10 bg-white/[0.02] px-3 py-2 text-[11px] text-white/50">
                ...
                <span>{secoes.length} cartões</span>
            ...
            <div ref={resultadoRef} className={card}>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[11px] text-white/40">
                    {validacao.validadas}/{validacao.totalClinicas} respostas citadas pelo código
              ...
              <div className="space-y-3">
                {secoes.map((sec, i) => {
                  ...
                  const conferidasCartao = itens.filter((it) => it.exigeFonte && it.status !== "valida" && it.conferido);
                  return (
                    <div key={sec.secao + i} className="rounded-xl border border-white/10 bg-white/[0.02] p-3">
                      <div className="mb-1.5 flex items-start gap-2">
                        ...
                        {conferidasCartao.length > 0 && <span className="inline-flex shrink-0 items-center gap-1 rounded-full border border-accent/40 bg-accent/10 px-2 py-0.5 text-[10px] font-medium text-accent"><CheckCircle2 className="h-3 w-3" /> {conferidasCartao.length} conferida{conferidasCartao.length > 1 ? "s" : ""}</span>}
                      </div>
                      <label className="mb-1 block text-[10px] uppercase tracking-[0.1em] text-white/30">Verso (resposta)</label>
                      <textarea className={inputCls + " min-h-[60px] resize-y font-mono text-[12px]"} value={textoEditado[sec.secao] ?? renderVerso(sec)} onChange={(e) => setTextoEditado((prev) => ({ ...prev, [sec.secao]: e.target.value }))} />
                      {problemas.length > 0 && (
                      ...
                      {conferidasCartao.length > 0 && (
                        <ul className="mt-1.5 space-y-1">
                          {conferidasCartao.map((p) => (
              ...
              <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-[11px] text-white/45">Um segundo modelo audita os cartões: a resposta responde à pergunta, citações reais, dados fiéis e sem extrapolação. Não reescreve sozinho — aponta e sugere.</p>
                  </div>
                  <button type="button" onClick={revisarAgora} disabled={revisando || gerando || busy} className="inline-flex shrink-0 items-center gap-2 rounded-full border border-accent/40 bg-accent/10 px-4 py-2 text-sm font-semibold text-accent transition hover:bg-accent/20 disabled:opacity-50">
                    {revisando ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />} {revisao ? "Revisar de novo" : "Revisar cartões"}
                  </button>
                </div>
              ...
              {/* Aplicar correções da IA — reancora as afirmações do verso reprovadas e sobe a confiança */}
              {(() => {
                  ...
                  <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.02] p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-0.5 text-[11px] text-white/45">A IA tenta reancorar as respostas reprovadas num trecho verbatim da referência (ou ajustar o texto pra bater, ou marcar honestamente como sem fonte). O código revalida e a confiança sobe conforme as referências cobrem. Não salva — revise e salve depois.</p>
                      </div>
          ...
          <div className={card}>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {statusAtual === "published" && (
                <>
                  <a href={`/flashcards/${doc.slug}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/[0.04] px-4 py-1.5 text-xs font-medium text-white/75 transition hover:text-white"><Globe className="h-3.5 w-3.5" /> Ver no site</a>
            ...
            <p className="mt-2 text-[11px] text-white/35">Publicar trava a versão mais recente (imutável) e a torna pública em /flashcards. Editar depois cria um novo rascunho; a versão pública só muda quando você republicar.</p>
          </div>
        </>
      )}
