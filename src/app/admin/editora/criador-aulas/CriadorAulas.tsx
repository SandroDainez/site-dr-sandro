...
export default function CriadorAulas({ docsIniciais, modo }: { docsIniciais: Doc[]; modo: "mock" | "real" }) {
  ...
  const [blocos, setBlocos] = useState<BlocoStatus[]>(AULA_BLOCOS.map(() => ({ status: "pendente" })));
  ...
  async function carregarVersoes(id: string, autoAbrirUltima = false) {
    ...
    if (r.ok) {
      setVersoes(r.data);
      // Ao abrir uma aula, recarrega a versão mais recente no editor (SEM entrar no modo edição focado).
  ...
  function abrirDoc(d: Doc) {
    ...
    setBlocos(AULA_BLOCOS.map(() => ({ status: "pendente" }))); setTextoEditado({});
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 1) AULA */}
      <div className={card}>
        <p className="mb-3 text-xs font-semibold uppercase tracking-[0.12em] text-white/40">1 · Aula</p>
        {doc ? (
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
              <div>
                <label className={labelCls}>Ou criar nova</label>
                <input className={inputCls} value={novoTitulo} onChange={(e) => setNovoTitulo(e.target.value)} placeholder="Título da aula. Ex.: Manejo da via aérea difícil" />
              </div>
      ...
      {doc && (
        <>
          {/* 2) REFERÊNCIAS + 3) ÁREA/PÚBLICO/GERAÇÃO — escondidos no modo edição focado */}
          {!editandoVersao && (<>
          <div className={card}>
          ...
          {/* 3) ÁREA + PÚBLICO + GERAÇÃO */}
          <div className={card}>
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.12em] text-white/40">3 · Área · Público-alvo · Geração</p>
            ...
            <div className="space-y-1.5">
              {AULA_BLOCOS.map((secoesDoBloco, i) => {
                const st = blocos[i];
                return (
            ...
            {metas.length > 0 && (
              <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg border border-white/10 bg-white/[0.02] px-3 py-2 text-[11px] text-white/50">
                ...
                <span>{metas.length}/{AULA_BLOCOS.length} blocos</span>
          ...
          {secoes.length > 0 && validacao && (
            <div ref={resultadoRef} className={card}>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-white/40">4 · Resultado (editável)</p>
              ...
              <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-[11px] text-white/45">Um segundo modelo audita a aula: objetivos cobertos pelo conteúdo, citações apontando para referências reais, dados fiéis e extrapolações. Não reescreve sozinho — aponta e sugere.</p>
                  </div>
                  <button type="button" onClick={revisarAgora} disabled={revisando || gerando || busy} className="inline-flex shrink-0 items-center gap-2 rounded-full border border-accent/40 bg-accent/10 px-4 py-2 text-sm font-semibold text-accent transition hover:bg-accent/20 disabled:opacity-50">
                    {revisando ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />} {revisao ? "Revisar de novo" : "Revisar aula"}
                  </button>
                </div>
                  ...
                  <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.02] p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-0.5 text-[11px] text-white/45">A IA tenta reancorar as afirmações reprovadas num trecho verbatim da referência (ou ajustar o texto pra bater, ou marcar honestamente como sem fonte). O código revalida e a confiança sobe conforme as referências cobrem. Não salva — revise e salve depois.</p>
                      </div>
          ...
          <div className={card}>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {statusAtual === "published" && (
                <>
                  <a href={`/aulas/${doc.slug}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/[0.04] px-4 py-1.5 text-xs font-medium text-white/75 transition hover:text-white"><Globe className="h-3.5 w-3.5" /> Ver no site</a>
            ...
            <p className="mt-2 text-[11px] text-white/35">Publicar congela a versão mais recente (imutável) e a torna pública em /aulas. Editar depois cria um novo rascunho; a versão pública só muda quando você republicar.</p>
          </div>
        </>
      )}
