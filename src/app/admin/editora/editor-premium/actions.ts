...
import { buildEditorPremiumPrompt } from "@/lib/ai/prompts/editor-premium";
...
import { buildCorrecaoPremiumPrompt, type ItemCorrigir } from "@/lib/ai/prompts/correcao-premium";
...
// Editor Premium — refinamento (densidade/forma) de um RASCUNHO a partir das referências.
// Reaproveita a biblioteca científica (sci_docs / sci_versions / sci_sources, migration 004)
// e o pipeline validado (DeepSeek gera/refina + GPT-4o revisa; confidence pelo CÓDIGO).
// module_type = 'editor-premium' distingue a auditoria em ai_generations.
...
export async function criarDoc(input: { title: string; especialidadeModulo: string }): Promise<Result<DocResumo>> {
  try {
    ...
    const { data, error } = await supabase.from("sci_docs")
      .insert({ title, slug, specialty: mapEspecialidadeDB(input.especialidadeModulo), status: "draft", stage: "editor-premium" })
      .select("id,title,slug,status,specialty").single();
    if (error) throw error;
    revalidatePath("/admin/editora/editor-premium");
    return { ok: true, data: data as DocResumo };
  } catch (e) { return { ok: false, error: msg(e) }; }
}
...
// Refina UM bloco de seções a partir do RASCUNHO + referências + seções já refinadas.
export async function gerarBloco(input: {
  docId: string; blocoIndex: number; especialidade: string; rascunho: string; secoesAnteriores: SecaoGerada[];
  ...
  try {
    ...
    if (sources.length === 0) return { ok: false, error: "Adicione ao menos uma referência antes de refinar." };

    const prompt = buildEditorPremiumPrompt({
      especialidade: input.especialidade, rascunho: input.rascunho, sources, secoesAlvo: bloco, secoesAnteriores: input.secoesAnteriores,
    });
    const provider = getProvider(aiProviders().generation); // mock | deepseek (AI_PROVIDER)
    const res = await provider.generate({
      modulo: "editor-premium", especialidade: input.especialidade,
...
// ESTÁGIO 2 — revisão do texto refinado (GPT-4o via dispatcher científico; mock no piloto).
...
export async function revisar(input: { docId: string; secoes: SecaoGerada[] }): Promise<Result<RevisaoResultado>> {
  try {
    ...
    if (!sres.ok) return sres;
    const sources = sres.data;
    if (!input.secoes?.length) return { ok: false, error: "Refine o texto antes de revisar." };
    const provider = getProvider(aiProviders().review);
    const rev = await provider.review({
      modulo: "editor-premium",
...
export async function salvarVersao(input: {
  ...
  try {
    ...
    await supabase.from("sci_docs").update({ current_version_id: versionId, stage: "editor-premium" }).eq("id", input.docId);

    if (input.geracoes?.length) {
      await supabase.from("ai_generations").insert(input.geracoes.map((g) => ({
        sci_version_id: versionId,
        module_type: "editor-premium",
        provider: g.provider,
        model: g.model,
    ...
    if (input.revisao) {
      ...
      await supabase.from("ai_generations").insert({
        sci_version_id: versionId,
        module_type: "editor-premium:review",
        provider: r.provider,
        model: r.model,
...
export async function publicarDoc(docId: string): Promise<Result<{ status: string; slug: string }>> {
  try {
    ...
    await sincronizarBiblioteca(supabase, {
      modulo: "editor-premium", tabelaOrigem: "sci_docs", docId,
...
export async function excluirDoc(id: string): Promise<Result<null>> {
  try {
    ...
    revalidatePath("/biblioteca-cientifica"); revalidatePath("/admin/editora/editor-premium");
...
export async function aplicarCorrecoes(input: { docId: string; secoes: SecaoGerada[] }): Promise<Result<{ secoes: SecaoGerada[]; validacao: Validacao; corrigidas: number; total: number }>> {
  try {
    ...
    if (!sres.ok) return sres;
    const sources = sres.data;
    if (!input.secoes?.length) return { ok: false, error: "Refine o texto antes de corrigir." };
    ...
    const { correcoes } = await corrigirCitacoes({ itens, sources, prompt: buildCorrecaoPremiumPrompt({ itens, sources }) });
