...
export default function EditorPremium({ docsIniciais, modo }: { docsIniciais: Doc[]; modo: "mock" | "real" }) {
  ...
  // foco=true → entra no MODO EDIÇÃO focado (esconde referências/rascunho/refinamento, mostra banner).
  ...
  function abrirDoc(d: Doc) {
    setDoc(d); setSecoes([]); setMetas([]); setSalvo(null); setError(null); setRevisao(null); setRascunho(""); setEditandoVersao(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {doc && (
        <>
          {/* 2) REFERÊNCIAS + 3) RASCUNHO + 4) REFINAMENTO — escondidos no modo edição focado */}
          {!editandoVersao && (<>
          <div className={card}>
          ...
          <div className={card}>
            ...
            <div className="mb-4 flex flex-wrap items-end gap-3">
              ...
              <button type="button" onClick={refinarTudo} disabled={gerando || busy || sources.length === 0} className="inline-flex items-center gap-2 rounded-full bg-accent px-5 py-2.5 text-sm font-semibold text-on-accent transition hover:brightness-110 disabled:opacity-40 disabled:cursor-not-allowed disabled:brightness-75">
          ...
          <div className={card}>
            ...
            <p className="mt-2 text-[11px] text-white/35">Publicar trava a versão mais recente (imutável) e a torna pública em /biblioteca-cientifica. Editar depois cria um novo rascunho; a versão pública só muda quando você republicar.</p>
          </div>
        </>
      )}
