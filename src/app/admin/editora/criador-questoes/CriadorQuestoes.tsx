...
export default function CriadorQuestoes({ docsIniciais, modo }: { docsIniciais: Doc[]; modo: "mock" | "real" }) {
  ...
  function addSource() {
    ...
    startTransition(async () => {
      ...
      if (r.ok) { setSources((prev) => [...prev, r.data]); setSTitulo(""); setSAutor(""); setSAno(""); setSTexto(""); } else setError(r.error);
    });
  }
        ...
        <>
          ...
          <div className={card}>
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-3 space-y-2">
              <p className="text-[11px] text-white/40">Adicionar referência (texto colado). É contra este texto que gabarito e justificativa são verificados.</p>
            ...
            <div className={card}>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[11px] text-white/40">{validacao.validadas}/{validacao.totalClinicas} justificativas com citação validada · {validacao.semFonte} sem fonte · {validacao.invalidas} inválidas</p>
                </div>
              </div>

              <div className="space-y-3">
                {questoes.map((q, i) => {
                  const itens = validacao.itens.filter((it) => it.secao === q.enunciado);
