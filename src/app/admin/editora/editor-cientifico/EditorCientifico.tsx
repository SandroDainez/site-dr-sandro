...
export default function EditorCientifico({ docsIniciais, modo }: { docsIniciais: Doc[]; modo: "mock" | "real" }) {
  ...
  async function gerarDoBloco(start: number) {
    ...
    for (let i = start; i < SCI_BLOCOS.length; i++) {
      ...
      const r = await gerarBloco({ docId: doc.id, blocoIndex: i, especialidade, secoesAnteriores: acumulado });
      if (!r.ok) {
      ...
      <div className={card}>
        ...
        {doc ? (
          ...
          <div className="space-y-3">
            {docs.length > 0 && (
              <div>
                <label className={labelCls}>Abrir existente (biblioteca)</label>
            ...
            <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
              <div>
                ...
                <input className={inputCls} value={novoTitulo} onChange={(e) => setNovoTitulo(e.target.value)} placeholder="Título do texto. Ex.: Corticoide na SDRA — evidência atual" />
              </div>
        ...
        <>
          ...
          <div className={card}>
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-3 space-y-2">
              <p className="text-[11px] text-white/40">Adicionar referência (texto colado). PDF fica para fase posterior.</p>
              ...
              <textarea className={inputCls + " min-h-[90px] resize-y"} value={sTexto} onChange={(e) => setSTexto(e.target.value)} placeholder="Cole aqui o texto do artigo/diretriz (é contra este texto que as citações são verificadas)." />
          ...
          <div className={card}>
            ...
            <div className="mb-4 flex flex-wrap items-end gap-3">
              ...
              <button type="button" onClick={gerarTudo} disabled={gerando || busy || sources.length === 0} className="inline-flex items-center gap-2 rounded-full bg-accent px-5 py-2.5 text-sm font-semibold text-on-accent transition hover:brightness-110 disabled:opacity-50">
                {gerando ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />} Gerar texto ({SCI_BLOCOS.length} blocos)
              </button>
            </div>
              ...
              <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-[11px] text-white/45">Um segundo modelo audita o texto montado: consistência entre seções, citações apontando para referências reais, dados transcritos fielmente e extrapolações. Não reescreve sozinho — aponta e sugere.</p>
                  </div>
