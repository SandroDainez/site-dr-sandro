...
import type { UtilAppData } from "@/lib/content";
import { saveUtilApps } from "@/app/admin/actions";
import RichTextEditor from "@/components/admin/RichTextEditor";

const ICON_OPTIONS = [
  "Wallet", "PiggyBank", "ListChecks", "CalendarClock", "Sparkles", "Layers",
  "FileText", "Zap", "BookOpen", "ShieldCheck",
];

type Props = { initialApps: UtilAppData[] };

export default function AppsUteisEditor({ initialApps }: Props) {
  const [apps, setApps] = useState<UtilAppData[]>(initialApps);
  const [isPending, startTransition] = useTransition();
  const [saved, setSaved] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [uploadingIndex, setUploadingIndex] = useState<number | null>(null);

  function update(index: number, field: keyof UtilAppData, value: string) {
    setApps((prev) => prev.map((a, i) => (i === index ? { ...a, [field]: value } : a)));
    setSaved(false);
  }
  ...
  async function handleImageUpload(index: number, file: File) {
    ...
    try {
      const blob = await upload(`apps-uteis/${Date.now()}-${file.name}`, file, {
        access: "private",
  ...
  function addApp() {
    setApps((prev) => [...prev, { title: "", categoria: "", desc: "", icon: "Wallet", link: "" }]);
    setSaved(false);
  }
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await saveUtilApps(apps);
    ...
    <div className="space-y-4">
      {apps.map((app, index) => (
        <div key={index} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs uppercase tracking-[0.1em] text-muted">Categoria (badge)</label>
              <input
                type="text"
                value={app.categoria}
                onChange={(e) => update(index, "categoria", e.target.value)}
                placeholder="Finanças, Organização..."
                className={inputCls}
              />
            </div>
            <div>
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        >
          ...
          {isPending ? "Salvando..." : "Salvar apps do dia a dia"}
        </button>
