...
import { getAulaPublicada, type AulaConteudo } from "@/lib/aulas-editora";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const a = await getAulaPublicada(slug);
  if (!a) return { title: "Aula não encontrada" };
  return { title: `${a.title} — Aula`, description: AVISO };
}

function textoSecao(secao: string, conteudo: AulaConteudo): string {
...
export default async function AulaPublicaPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const [aula, header, navItems, typo, navStyle] = await Promise.all([
    getAulaPublicada(slug), getHeader(), getNavItems(), getTypography(), getNavStyle(),
  ]);
  if (!aula) notFound();

  const secoes = (aula.conteudo.secoes ?? []).map((s) => s.secao);
  const referencias = aula.conteudo.referencias ?? [];
  const publico = aula.conteudo.publicoAlvo;
    ...
    <div className="min-h-screen bg-[#0f1420] text-white">
      ...
      <header data-typo="header" className="sticky top-0 z-50 border-b border-white/10 bg-[#0f1420]/80 backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-7xl flex-col items-center gap-3 px-6 py-4 lg:flex-row lg:justify-between lg:gap-0">
          ...
          <Link href="/aulas" className="flex items-center gap-1 text-sm text-white/50 transition hover:text-white lg:hidden">← Aulas</Link>
        </div>
      </header>

      <main className="mx-auto w-full max-w-6xl px-6 py-12">
        <Link href="/aulas" className="mb-6 inline-flex items-center gap-1.5 text-sm text-white/50 transition hover:text-white">← Todas as aulas</Link>
        ...
        <div className="mb-6">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-accent">Aula · {ESP_LABEL[aula.specialty] ?? aula.specialty}</p>
          <h1 className="mt-2 text-3xl font-semibold leading-tight tracking-tight md:text-4xl">{aula.title}</h1>
          <div className="mt-2 flex flex-wrap items-center gap-3 text-[12px] text-white/40">
            {publico && <span className="inline-flex items-center gap-1"><Users className="h-3.5 w-3.5" /> {publico}</span>}
            {aula.publicado_em && <span>Publicado em {dataCurta(aula.publicado_em)}</span>}
          </div>
        </div>
        ...
        <div className="lg:grid lg:grid-cols-[220px_1fr] lg:gap-10">
          ...
          <article className="min-w-0 space-y-6">
            {(aula.conteudo.secoes ?? []).map((sec) => {
              const texto = textoSecao(sec.secao, aula.conteudo);
              return (
