...
// Prompt da CORREÇÃO ("Aplicar correções da IA") do Editor Premium. Recebe as afirmações
// cuja citação NÃO foi validada e tenta reancorá-las num trecho verbatim da referência —
// ou ajustar o texto pra bater — ou marcar honestamente como sem fonte. O CÓDIGO revalida
// depois (âncora inventada não cola), então este prompt não tem como "trapacear".
// Baseado em prompts/correcao-protocolos.ts, com wording adaptado ao texto científico
// refinado do Editor Premium (mesmo JSON de saída, mesmas regras).

export const CORRECAO_PREMIUM_PROMPT_VERSION = "1.0.0";
...
export function buildCorrecaoPremiumPrompt(args: { itens: ItemCorrigir[]; sources: Source[] }): string {
  const { itens, sources } = args;
  return `Você corrige as CITAÇÕES de afirmações de um texto científico refinado (Editor Premium).
Cada afirmação abaixo teve a citação REPROVADA (a âncora não foi encontrada literalmente no texto
da referência). Para CADA item, faça UMA opção, nesta ordem de preferência:
...
(3) Se NENHUMA referência sustenta a afirmação: source_id=null, ancora=null, tipo="geral". NÃO
    invente âncora nem force citação — honestidade acima de tudo (rigor científico > completude).
