...
// Prompt da CORREÇÃO ("Aplicar correções da IA") do Editor Científico. Mesma lógica do
// Arquiteto de Protocolos (ver prompts/correcao-protocolos.ts) — só o wording muda para
// "texto científico" em vez de "protocolo clínico institucional". Recebe as afirmações
...
export const CORRECAO_CIENTIFICO_PROMPT_VERSION = "1.0.0";
...
export function buildCorrecaoCientificoPrompt(args: { itens: ItemCorrigir[]; sources: Source[] }): string {
  const { itens, sources } = args;
  return `Você corrige as CITAÇÕES de afirmações de um texto científico (artigo/revisão para publicação
médica). Cada afirmação abaixo teve a citação REPROVADA (a âncora não foi encontrada literalmente no
texto da referência). Para CADA item, faça UMA opção, nesta ordem de preferência:
...
(3) Se NENHUMA referência sustenta a afirmação: source_id=null, ancora=null, tipo="geral". NÃO
    invente âncora nem force citação — honestidade acima de tudo (rigor científico).
