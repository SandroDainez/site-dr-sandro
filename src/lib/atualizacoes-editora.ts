...
export type AtualizacaoResumo = { id: string; title: string; slug: string; specialty: string; areas: string[] };
...
export async function getAtualizacoesPublicadas(): Promise<AtualizacaoResumo[]> {
  ...
  try {
    ...
    const { data } = await supabase.from("protocol_update_docs").select("id,title,slug,specialty,areas").eq("status", "published").order("updated_at", { ascending: false });
