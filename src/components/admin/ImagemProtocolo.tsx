...
import { getImagemProtocolo, definirImagemProtocolo } from "@/app/admin/editora/arquiteto-protocolos/actions";

// Imagem/infográfico do protocolo (aparece no card padrão em /protocolos, home e hubs).
// Autossuficiente: carrega a imagem atual e salva no upload/remover.
export default function ImagemProtocolo({ protocolId }: { protocolId: string }) {
  ...
  useEffect(() => {
    let vivo = true;
    getImagemProtocolo(protocolId).then((r) => { if (vivo && r.ok) setUrl(r.data); });
    return () => { vivo = false; };
  }, [protocolId]);
  ...
  async function remover() {
    setErro(null); setBusy(true);
    const r = await definirImagemProtocolo(protocolId, null);
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
      ...
      <p className="mb-3 text-xs text-white/50">Aparece no card do protocolo em /protocolos, na home e nos hubs. Sem imagem, o card usa o ícone padrão.</p>
      {url ? (
