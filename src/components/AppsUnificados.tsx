...
import { FINALIDADES, type Finalidade } from "@/lib/finalidades";

// Seção unificada de aplicativos, agrupada por FINALIDADE (pra que serve). Cada card traz
// seu selo Grátis/Assinatura e (em utilidades) um rótulo livre. Fonte: getAplicativos().
...
function Card({ app }: { app: AplicativoData }) {
  const pago = app.acesso === "assinatura";
  const Icone = ICONE_FINALIDADE[app.finalidade] ?? Wrench;
  const inner = (
    <>
      <div className={`pointer-events-none absolute inset-x-0 top-0 h-28 bg-gradient-to-b ${pago ? (app.glow || "from-accent-blue/15") : "from-emerald-500/10"} to-transparent`} />
      ...
      <div className="relative">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {app.rotulo && <span className="rounded-full border border-white/15 bg-white/[0.05] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-white/60">{app.rotulo}</span>}
            {pago ? (
        ...
        {app.text && <p className="mt-4 text-sm leading-relaxed text-muted" dangerouslySetInnerHTML={{ __html: sanitizeRichText(app.text) }} />}
        {app.highlights && app.highlights.filter(Boolean).length > 0 && (
          <div className="mt-4 space-y-1.5">
            {app.highlights.filter(Boolean).map((h) => (
        ...
        <span className="mt-6 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-white transition group-hover:scale-[1.02] group-hover:bg-white/10 group-hover:shadow-[0_0_30px_rgba(95,143,255,0.35)]">
          {pago ? "Ver detalhes" : app.link ? "Abrir app" : "Em breve"} <ArrowRight className="h-3.5 w-3.5" />
        </span>
      </div>
    </>
  );
...
export default function AppsUnificados({ aplicativos, cols = 3 }: { aplicativos: AplicativoData[]; cols?: number }) {
  const grupos = FINALIDADES
    .map((f) => ({ ...f, itens: aplicativos.filter((a) => a.finalidade === f.valor) }))
      ...
      {grupos.map((g) => (
        <div key={g.valor}>
          ...
          <div className="card-grid items-start gap-5" style={colStyle(cols)}>
            {g.itens.map((app) => <Card key={app.id} app={app} />)}
          </div>
        </div>
      ))}
    </div>
